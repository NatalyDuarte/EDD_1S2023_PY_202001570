...
class ArbolAVL {
    ...
    graphHash() {
        let text = ""
        Hash.table.forEach(prob => {
            text += '<tr><td><center>' + prob.carnet + '</center></td><td><p style="text-align: justify">' + prob.nombre + '</p></td><td><p style="text-align: justify">' + prob.password + '</p></td></tr>\n'
        })
        return text;
    }
    grafEnOrdenSelect(Nodo) {
        if (Nodo == null) return "";
        let text = ""
        text += this.grafEnOrdenSelect(Nodo.izquierda);
        text += ' <option value="' + Nodo.dato.carnet + '">' + Nodo.dato.carnet + '</option>'
        text += this.grafEnOrdenSelect(Nodo.derecha);
        return text
    }
    grafEnOrdenLR(Nodo) {
        if (Nodo == null) return "";
